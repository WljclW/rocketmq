...
public class MixAll {
    ...
    public static void printObjectProperties(final Logger logger, final Object object,
        ...
        Field[] fields = object.getClass().getDeclaredFields();     //反射拿出所有字段
        for (Field field : fields) {    //for干的就是拿出满足指定条件的字段并进行打印name和value
        ...
        for (Method method : methods) {
            ...
            if (mn.startsWith("set")) {
                try {
                    ...
                    String key = first.toLowerCase() + tmp; //经过此步会把大写的C变小写，即方法名setConfigStorePath变为configStorePath
                    String property = p.getProperty(key);  //看Properties文件是不是
